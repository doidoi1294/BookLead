<x-app-layout>
    <x-slot name="title">日記作成ページ</x-slot>
    <x-slot name="header">Blog Name</x-slot>
    
    <div class="max-w-md mx-auto mt-8">
        <form action="/diaries" method="POST" class="space-y-4">
            @csrf
            <div class="book">
            ...
            <div class="date">
                ...
                <input name="diary[date]" type="date" class="border border-gray-300 rounded-md py-2 px-3 focus:outline-none focus:border-blue-500" value="{{ old('diary.date', date('Y-m-d')) }}">
                <p class="date__error mt-1 text-red-500">{{ $errors->first('diary.date') }}</p>
            </div>
            <div class="page">
                <h2 class="text-lg font-semibold">読んだページ数</h2>
                <input type="number" name="diary[page]" class="border border-gray-300 rounded-md py-2 px-3 focus:outline-none focus:border-blue-500" placeholder="ページ数を入力(半角数字)" value="{{ old('diary.page') }}">
                <p class="page__error mt-1 text-red-500">{{ $errors->first('diary.page') }}</p>
            </div>
            <div class="body">
                <h2 class="text-lg font-semibold">読書日記</h2>
                <textarea name="diary[body]" class="border border-gray-300 rounded-md py-2 px-3 focus:outline-none focus:border-blue-500 w-full h-32 resize-none">{{ old('diary.body') }}</textarea>
